...
class TSN(nn.Module):
    ...
    def _prepare_base_model(self, base_model):
        ...
        if 'resnet' in base_model:
            # from ops.resnet import ResNet
            # self.base_model = getattr(torchvision.models, base_model)(True if self.pretrain == 'imagenet' else False)
            self.base_model = resnet(base_model)
            if self.is_shift:
    ...
    def forward(self, input, no_reshape=False):
        ...
        if self.reshape:
            ...
            if self.modality == 'RTD':
                ...
                output = (output_rgb + output_ir + 0.2*output_depth) / 3.0

            # if self.modality == 'RTD':
            #     output_rgb = self.consensus(base_out[:base_out.shape[0] // 2,])
            #     output_ir = self.consensus(base_out[base_out.shape[0] // 2:base_out.shape[0] // 2 *2,])
            #     output = (output_rgb + output_ir ) / 2.0
...
def resnet(base_model):
    if ('18' in base_model):
        model = resnet18(pretrained=True,num_segments=8)
    elif ('34' in base_model):
        model = resnet34(pretrained=True,num_segments=8)
    elif ('50' in base_model):
        model = resnet50(pretrained=True,num_segments=8)
    elif ('101' in base_model):
        model = resnet101(pretrained=True,num_segments=8)
    elif ('152' in base_model):
        model = resnet152(pretrained=True,num_segments=8)
    return model
